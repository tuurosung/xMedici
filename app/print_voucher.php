...
<?php
if(!isset($_SESSION['print_this_transaction'])){
    $transaction_id=clean_string($_GET['transaction_id']);
}
else {
  $transaction_id=$_SESSION['print_this_transaction'];
}


$transaction_info=mysqli_query($db,'SELECT * FROM sales WHERE transaction_id="'.$transaction_id.'" && subscriber_id="'.$active_subscriber.'"') or die('failed');
$transaction_info=mysqli_fetch_array($transaction_info);

// $waiter_info=waiter_info($order_details['waiter']);

 ?>


<div class="container">
  <div class="font-weight-bold" style="font-weight:bold; margin:1rem 0rem 1rem">CASH SALE RECEIPT</div>
